...
<div align="center">
  <h3>A Java Console Application Demonstrating Core Data Structures</h3>
</div>
...
1. **Custom Linked List (`CrewList.java`, `AstronautNode.java`)** — Used to store the crew roster dynamically.
2. **Circular/Linear Queue (`MissionQueue.java`)** — Hand-coded queue array to dispatch missions in chronological order.
3. **Stack (`EmergencyStack.java`)** — Hand-coded stack array to manage and resolve the most recent emergencies first.
4. **Selection Sort algorithm** — Integrated into the Linked List to sort nodes by integer data (`experience`).
